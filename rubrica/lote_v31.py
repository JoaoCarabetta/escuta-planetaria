#!/usr/bin/env python3
"""Serve e grava anotações da rubrica v3.1. Vários anotadores em paralelo.
...
O anotador é gravado como `claude:v31:agenteN`, ao lado das anotações v3 do
...
PORTAO = ['literal', 'figurado', 'devaneio', 'obra', 'noticia', 'propaganda']
MARCAS = ['meta', 'copy_paste', 'falta_contexto', 'bandeira', 'descartavel']
LISTAS = ['figura', 'carga', 'tom', 'despertar', 'memoria', 'modo',
          'presencas', 'atribuicao']
...
def gravar(agente):
    ...
    for d in dados:
        ...
        extra = {k: p[k] for k in ('obra', 'noticia', 'propaganda',
                                   'copy_paste', 'falta_contexto', 'bandeira')}
        extra.update({k: lista(d.get(k)) for k in
                      ('despertar', 'memoria', 'presencas', 'atribuicao', 'modo')})
        c.execute("""INSERT OR REPLACE INTO anotacoes_v3
            (relato_id, anotador, tem_literal, tem_figurado, tem_devaneio,
             ...
             nota, bolsa, extra)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'v3.1',?)""",
            (d['id'], f'claude:v31:agente{agente}',
             ...
             json.dumps(lista(d.get('modo')), ensure_ascii=False),
